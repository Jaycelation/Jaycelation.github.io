...
An attacker can force the Master Baker/admin account to share its private recipe box without consent.

The practical impact is:

- Unauthorized access to admin-only vault content.
- Disclosure of private recipe data.
- Execution of a state-changing action on behalf of another authenticated user.
- In the challenge context, leakage of the flag.
...
The application should implement CSRF protection for all state-changing endpoints, especially `/share`.

Recommended mitigations:

- Add a server-side CSRF token to the share form.
- Validate the CSRF token on every `POST /share` request.
- Reject requests with missing or invalid CSRF tokens.
- Set the session cookie to `SameSite=Lax` or `SameSite=Strict` where possible.
- Validate the `Origin` and/or `Referer` header for sensitive state-changing actions.
- Consider adding an explicit confirmation step before sharing private vault data.

## Conclusion

The vulnerability was caused by a sensitive state-changing endpoint that relied only on cookies for authentication and did not verify user intent.

By combining the missing CSRF protection on `/share` with the Master Baker bot's URL visit behavior, I could force the admin account to share its private vault with my account.

Once the vault was shared, the flag was accessible through the normal `/vault` interface.
